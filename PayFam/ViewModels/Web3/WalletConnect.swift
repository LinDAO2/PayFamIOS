...
class WalletConnect {
    ...
    func connect() -> String {
        let wcUrl =  WCURL(topic: UUID().uuidString,
                           ...
                           key: randomKey())
        let clientMeta = Session.ClientMeta(name: "Payfam",
                                            description: "Banking the unbanked for a brighter future",
                                            icons: [],
                                            url: URL(string: "https://payfam.io")!)
        let dAppInfo = Session.DAppInfo(peerId: UUID().uuidString,
                                        peerMeta: clientMeta,
//                                        chainId: 137) // Polygon chain
                                        chainId: 80001) // Mumbai chain
